...
- [T028 - Pronosticar el volumen diario de clusters con reclamo COM](tareas/T028_pronosticar_y_explicar_volumen_diario_reclamos_com.md) - prioridad alta. El ensayo de dia de semana en la correccion no mejora test de forma estable; resta validar el candidato, definir su contrato operativo e integrarlo en la app sin mezclarlo con el ranking espacial.
- [T030 - Refactorizar la app operativa Shiny](tareas/T030_refactorizar_app_operativa_shiny.md) - prioridad alta. La app v2 esta desplegada y visible en Cloudera; quedan la lectura barrial y la definicion de metricas territoriales adicionales.
- [T029 - Explicar los dias con alta afectacion territorial COM](tareas/T029_explicar_dias_alta_afectacion_territorial_com.md) - prioridad media. El bootstrap conjunto del cuadratico completo respalda la curvatura y el signo negativo de atraso por cambio; el cambio lineal y NBI no quedan estables y resta decidir la especificacion final.
- [T032 - Actualizar datos y reevaluar modelos con test puro 2026](tareas/T032_actualizar_datos_y_reevaluar_modelos_2026.md) - prioridad alta. Pasos 1-6 completos y documentados en un informe preliminar; la seleccion final queda abierta y el test sigue sellado.
- [T013 - Diagnosticar cobertura territorial de Zona Limpia](tareas/T013_diagnosticar_cobertura_territorial_y_fuente_faltante_de_zona_limpia.md) - prioridad alta. La auditoria espacial recupera visitas del Municipio B que tenian `municipio` vacio; resta recalcular los diagnosticos historicos que lo excluyeron.
...
- [T026 - Reevaluar y consolidar el modelo Zona Limpia](tareas/T026_reevaluar_y_consolidar_el_modelo_zona_limpia.md) - prioridad alta. `operativo_segmento_demografia` mejora de forma pequena y consistente sin territorio nominal; resta medir el efecto de reincorporar Municipio B mediante la asignacion espacial.
...
- [T031 - Publicar el proyecto y completar la replica en Cloudera](tareas/T031_publicar_proyecto_y_replicar_en_cloudera.md) - prioridad alta. Codigo publicado, datos cargados por fuera de Git, dependencias instaladas y app v2 operativa en Cloudera.
