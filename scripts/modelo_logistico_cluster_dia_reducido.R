...
out_base <- Sys.getenv(
  "OUT_BASE_MODELO",
  unset = "data/processed/modelos/logistico_cluster_dia_reducido"
)
...
rezago_reclamos_dias <- as.integer(Sys.getenv("REZAGO_RECLAMOS_DIAS", unset = "1"))
...
unlink(file.path(out_base, c("predicciones_validacion", "metricas", "lift_validacion", "lift_diario_validacion", "calibracion_validacion", "coeficientes")), recursive = TRUE)
...
saveRDS(
  ...
  file = file.path(out_base, "modelo_logistico_reducido.rds")
)
...
print(data.table(
  ...
  usar_exceso_levante = usar_exceso_levante,
  predictores = length(features_modelo)
))
print(metricas)
