...
variables_continuas <- c(
  "log_densidad_pob", "porc_nbi_segmento", "media_h", "media_q_7"
)
...
if (!isTRUE(fit_q7$converged)) stop("El modelo inferencial q7 no convergio")

valid[, prob_predicha := as.numeric(predict(
  fit_q7, newdata = valid, type = "response"
))]
pred_q7 <- valid[, .(
  observado = sum(observado),
  predicho = sum(n_clusters * prob_predicha)
), by = dia]
pred_q7[, candidato := "inferencial_q7_sola"]

pred_actual <- fread(path_actual)[candidato == "inferencial_q30_ciclo"]
pred_actual[, dia := as.IDate(dia)]
predicciones <- rbindlist(list(
  pred_actual[, .(dia, candidato, observado, predicho)],
  pred_q7[, .(dia, candidato, observado, predicho)]
))
...
coeficientes <- as.data.table(
  summary(fit_q7)$p.table,
...
saveRDS(list(
  ...
  usa_test = FALSE,
  bootstrap = FALSE
), file.path(path_out, "inferencial_q7_sola.rds"))
...
fwrite(coeficientes, file.path(path_out, "coeficientes_q7_sola.csv"))
...
print(coeficientes[termino %in% c("z_media_h", "z_media_q_7")])
